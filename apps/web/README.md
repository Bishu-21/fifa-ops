...
Sensory network nodes only provide raw telemetry coefficients (e.g., `Density: 92%`). Intelligent support is needed to evaluate these coefficients alongside textual logs (e.g., `"Gate C turnstiles lost power"`) to synthesize clear, step-by-step guidance. It writes redirection plans and pre-translated scripts instantly, saving precious minutes during crowd surges.
